...
Phase 1が「妥当性を確認できた」と言えるのは、次の一点を満たしたときです。

> **局所情報・資源制約・一般化された行動のみから、既存技能・設備の別用途への再構成という現象が原理的に発生可能であること**

つまり、答えを与えず、神の視点を与えず、一般化されたActionしか持たないAgentの相互作用から、再構成が**起こりうる**ことをモデル上で示せれば、Phase 1は妥当です。

**明示的に基準に含めないもの:**

- **条件間の順序関係**（A > B > C のような大小関係）。これは Milestone 4 における観測結果であって、妥当性の条件ではありません。**差がないこと、事前の予想と逆転すること（例: A < B）も正当な結果**であり、モデルが妥当でない証拠にはなりません。
- **供給量の絶対値**
- **現実データとの数値一致**

**なぜ順序関係を基準に含めないのか:**

順序関係を妥当性の条件にすると、「その順序が出るまでモデルを調整する」ことが正当化されてしまいます。それは §30 Anti-Goal「結果が出るように後からパラメータを恣意的調整する」そのものです。妥当性判定と仮説検証を分離し、**妥当性は機構の成立可能性のみで判定し、仮説の成否は結果として報告する**という構造にします。
...
上記は**研究上の関心**であって、**Phase 1 の妥当性判定基準ではありません**。両者を混同してはいけません。

| | 内容 | 判定 |
|---|---|---|
| **妥当性判定基準**（§8） | 局所情報・資源制約・一般化された行動のみから、既存技能・設備の別用途への再構成が原理的に発生可能であること | Phase 2 へ進む条件 |
| **仮説検証の結果**（H1/H2/H3） | topology・peer learning が Maker形成・知識拡散・転化に差を生むか | 観測して報告するもの。**Phase 2 へ進む条件ではない** |

条件間に差が出なかった場合、あるいは事前の予想と逆転した場合（例: A < B）でも、機構が成立していれば Phase 1 は妥当です。その場合は H1・H2 に対する否定的な知見として報告します。

差が出るまでパラメータを調整してはいけません（§30）。
...
#### 改訂4: §8 / §29 — Phase 1 妥当性判定基準の追加

**旧状態（v0.1）:**

§8 は「現実に起きた転化現象を、個別具体的な行動命令なしに再現可能か検証する」とし、§9 は「Phase 1でモデル妥当性を確認した後」に Phase 2 へ進むとしていた。しかし**「再現できた」と判定する基準が本文に存在しなかった**。§29 の Phase 1 成功条件も、研究上の関心を述べるにとどまり、判定基準としては機能していなかった。
...
あわせて §29 に「成功条件（研究上の関心）」と「妥当性判定基準」の区別を明記した。
...
1. **反証不能性の解消** — 判定基準がないと「妥当性を確認した」という主張が反証不能になり、Phase 2 へ進む根拠が「それらしく見えたから」になる。
2. **順序関係を基準にしない理由** — `docs/REVIEW.md` §12.1【要承認3】は当初「条件間の順序関係が安定して観測されること」を基準案の第1項に含めていた。これは**採用しなかった**。順序関係を妥当性の条件にすると、その順序が出るまでモデルを調整することが正当化され、§30 Anti-Goal に直結する。妥当性判定と仮説検証は分離しなければならない。
3. **現実データとの数値一致を求めない理由** — COVID期のメイカー活動について、本モデルのパラメータに対応する定量データは存在しないか極めて限定的である。数値一致を基準にすると、達成不可能な基準になるか、恣意的なフィッティングになる。

これにより、§19 が明記する「**A < B も正当な研究結果**」という立場と、妥当性判定基準が整合する。

---

#### 改訂5: §27 Repository Structure — config ファイル名の追従
...
旧定義のファイル名は、改訂1で**廃止された条件名**（`Culture` / `Skill-Matched Random` / `Isolated`）に基づいていました。改訂1の追従漏れであり、**新規の研究命題変更ではありません**。

放置した場合、実装着手時に廃止済みの条件名でファイルを作成する事故が起こりえます。実験条件名は §19 の中核であり、ファイル名の取り違えは条件の取り違えに直結します。

なお §27 本文は「必要なら改善してください」としており、§0 が固定する研究命題・理論・実験原則には該当しません。本改訂はその範囲内の修正です。

**§27 の configs 以外の記載は変更していません。** `src/` 以下（`economy.py`、`logistics.py`、`transition.py` 等）は M3 以降で必要になるため §27 の記載どおり残します。M1 で実際に作成するモジュール構成は `docs/DESIGN_M1.md` §2 が担当します。
