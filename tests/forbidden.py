...
単純な部分文字列照合は偽陽性を生む。実際に "ppe" が "appeared" に一致した。
ASCII 語は**語境界**で照合し、日本語は境界がないため部分一致のままとする。

なお `\b` はハイフンを語境界として扱うため、"a-ppe-ared" のようなハイフン区切りは
今も "ppe" に一致する（意図どおり。分割による回避を許さない）。
語境界照合が防いでいるのは "appeared" のような**連続した文字列**の偽陽性である。
